...
CREATE FUNCTION deparser.import_foreign_schema_stmt ( node jsonb, context jsonb DEFAULT '{}'::jsonb ) RETURNS text AS $EOFCODE$
DECLARE
  output text[];
BEGIN
...
CREATE FUNCTION ast_helpers.denormalized_fields_trigger_body ( v_schema_name text, v_table_name text, v_table_field text, v_set_fields text[], v_ref_field text, v_ref_fields text[] ) RETURNS text AS $EOFCODE$
DECLARE
  ast_expr jsonb;
  body text;
  ref_fields jsonb[];
